...
for r=1:size(Y,2)
    ...
    if isempty(b)==0
        [f, u, bb]=ksdensity(Y{r},'bandwidth',b(r));
    elseif isempty(b)
        [f, u, bb]=ksdensity(Y{r});
    end
...
for r=1:size(Y,2)
    if isempty(lc) == 1
        if setX == 0
            h(r)=fill([F(:,r)+r;flipud(r-F(:,r))],[U(:,r);flipud(U(:,r))],fc(r,:),'FaceAlpha',alp,'EdgeColor','none');
        else
            h(r)=fill([F(:,r)+x(r);flipud(x(r)-F(:,r))],[U(:,r);flipud(U(:,r))],fc(r,:),'FaceAlpha',alp,'EdgeColor','none');
        end
    else
        if setX == 0
            h(r)=fill([F(:,r)+r;flipud(r-F(:,r))],[U(:,r);flipud(U(:,r))],fc(r,:),'FaceAlpha',alp,'EdgeColor',lc);
        else
            h(r)=fill([F(:,r)+x(r);flipud(x(r)-F(:,r))],[U(:,r);flipud(U(:,r))],fc(r,:),'FaceAlpha',alp,'EdgeColor',lc);
        end
    end
    hold on
    if setX == 0
        if plotmean == 1
            p(1)=plot([interp1(U(:,r),F(:,r)+r,MX(:,r)), interp1(flipud(U(:,r)),flipud(r-F(:,r)),MX(:,r)) ],[MX(:,r) MX(:,r)],mc,'LineWidth',2);
        end
        if plotmedian == 1
            p(2)=plot([interp1(U(:,r),F(:,r)+r,MED(:,r)), interp1(flipud(U(:,r)),flipud(r-F(:,r)),MED(:,r)) ],[MED(:,r) MED(:,r)],medc,'LineWidth',2);
        end
    elseif setX == 1
        if plotmean == 1
            p(1)=plot([interp1(U(:,r),F(:,r)+r,MX(:,r))+x(r)-r, interp1(flipud(U(:,r)),flipud(r-F(:,r)),MX(:,r))+x(r)-r],[MX(:,r) MX(:,r)],mc,'LineWidth',2);
        end
        if plotmedian == 1
            p(2)=plot([interp1(U(:,r),F(:,r)+r,MED(:,r))+x(r)-r, interp1(flipud(U(:,r)),flipud(r-F(:,r)),MED(:,r))+x(r)-r],[MED(:,r) MED(:,r)],medc,'LineWidth',2);
        end
    end
end
