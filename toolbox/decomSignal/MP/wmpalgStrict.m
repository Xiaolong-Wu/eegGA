function [YFIT,R,COEFF,IOPT,qual,X] = wmpalgStrict(Strict,AlgNAM,Y,varargin)
...
%   Parameter Strict is added to the original function so that its 
%   coefficients are allowed or not allowed to be negative in 'BMP'.
%   Wu Jiefu
%   Last Revision: 25-Oct-2021.
...
switch AlgNAM
    case 'BMP'   % Basic Matching Pursuit Algorithm.
        ...
        while ~stopALG
            ...
            % choose the max(abs(scalar product)
            if Strict==2
                [~,i]= max(R' * X);
            else
                [~,i]= max(abs(R' * X));
            ...
            if k>=itermax || curERR<valERR || (Strict==2 && max(R' * X)<=0)
                stopALG = true;
                kPLOT(kPLOT>=k) = [];
                kPLOT = [kPLOT,k]; 
            end
...
if isequal(typePLOT,'hor')  % Test horizontal plot
    ...
    legend(getWavMSG('Wavelet:wmp1dRF:Leg_Data'), ...
        getWavMSG('Wavelet:wmp1dRF:Leg_Fit'))
    ...
    function plotDEC
        ...
        legend(getWavMSG('Wavelet:wmp1dRF:Leg_Sig'), ...
            getWavMSG('Wavelet:wmp1dRF:Leg_App'));
        S1 =  num2str(qual(k));       
